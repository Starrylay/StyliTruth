...
if __name__ == "__main__":
    ...
    style_score = data[0]["score"] 
    truth_score = data[1]["score"]
    all_score = {}

    all_score["OA"] = float(style_score["si_score"]) * float(style_score["sp_score"]) * float(style_score["fs_score"])
    all_score["TI"] = float(truth_score["avg_both_acc"])
  
    all_score["S-TI"] = 2* float(all_score["OA"]) * float(all_score["TI"]) / (float(all_score["OA"]) + float(all_score["TI"]))
    data[2]["score"] = all_score
